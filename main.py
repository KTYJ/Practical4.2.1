...
# Streamlit app
def main():
    st.title('Gender Prediction App')               # Add title
    st.write('Enter a name to predict its gender.') # Add input field
    ...
    if st.button('Predict'):
        if input_name.strip() != '':
            ...
            # Display prediction
            st.success(f'The predicted gender for "{input_name}" is: {predicted_gender}') #success message
        else:
